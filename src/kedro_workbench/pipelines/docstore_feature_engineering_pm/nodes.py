...
def extract_patch_managment_to_clean(
    document_limit,
    begin_docstore_feature_engineering=True
):
...
def generate_noun_phrases(data):
    if data.empty:
        return data
    data['noun_chunks'] = data['email_text_clean'].apply(
        feat_eng.get_noun_phrases
        )
    # pprint(data)
...
def generate_keywords(data):
    if data.empty:
        return data
    data['all_keywords'] = data['normalized_tokens'].apply(
        feat_eng.extract_keywords
        )
...
def filter_keywords(data):
    if data.empty:
        return data
    data['filtered_keywords'] = data['all_keywords'].apply(
        feat_eng.filter_tuples
        )
...
def build_user_prompt_data_patch(data, metadata_keys=None):
    ...
    data['metadata_context'] = data.apply(
        lambda row: create_metadata_string_for_user_prompt(
            row,
            metadata_keys
            ),
        axis=1)
...
def fit_classification_prompt_patch(data, max_prompt_tokens):
    ...
    separator = "------------------------\n"
    data['user_prompt'] = data.apply(lambda row: fit_prompt_to_window(
...
def remove_mongo_duplicates_patch(update_flag):
    if update_flag:
        cursor = mongo.collection.aggregate([
            ...
            {
                "$group": {
                    "_id": {"collection": "$metadata.collection", "id": "$metadata.id"},
                    "count": {"$sum": 1},
                    "duplicates": {"$push": "$_id"}
                }
            },

...
from kedro_workbench.utils.nlp_feature_engineering_prompts import (
    completion_post_type_classify_system_prompt, completion_keyword_evaluation_system_prompt,completion_noun_chunk_evaluation_system_prompt, completion_post_type_classify_user_prompt
)
import kedro_workbench.utils.feature_engineering as feat_eng
from kedro_workbench.utils.llm_utils import evaluate_rake_keywords, evaluate_noun_chunks, classify_email, create_metadata_string_for_user_prompt, fit_prompt_to_window
...
from kedro_workbench.utils.db_utils import print_mongo_result_properties, remove_duplicates
...
def extract_patch_managment_to_clean(document_limit, begin_docstore_feature_engineering=True):
    ...
    print(f"num emails number to feature engineer: {len(list_of_dicts_to_clean)}")
...
def transform_feature_engineering_data_patch(docs, metadata_keys=None):
    if len(docs) == 1 and docs[0] == 'None':
        columns = ['id', 'topic', 'receivedDateTime', 'email_text_clean', 'unique_tokens', 'noun_chunks', 'normalized_tokens', 'all_keywords', 'filtered_keywords', 'evaluated_keywords', 'evaluated_noun_chunks', 'user_prompt', 'conversation_link', 'email_text', 'metadata_context', 'post_type']
...
def get_unique_token_count(data):
    if data.empty:
        return data
    data['unique_tokens'] = data['email_text_clean'].apply(lambda x: len(feat_eng.get_unique_tokens(x)))
    #pprint(data)
...
def build_lemmatized_tokens(data):
    if data.empty:
        return data
    data['normalized_tokens'] = data['email_text_clean'].apply(feat_eng.get_lemmatized_tokens)
...
def evaluate_keywords_node(model, data, max_tokens, temperature):
    ...
    data["evaluated_keywords"] = data.progress_apply(lambda x: evaluate_rake_keywords(model, x, system_prompt, max_tokens, temperature), axis=1)
    logger.info(f"LLM has evaluated keywords.")
...
def evaluate_noun_chunks_node(model, data, max_tokens, temperature):
    ...
    data["evaluated_noun_chunks"] = data.progress_apply(lambda x: evaluate_noun_chunks(model, x, system_prompt, max_tokens, temperature), axis=1)
    logger.info(f"LLM has evaluated noun chunks.")
...
def fit_classification_prompt_patch(data, max_prompt_tokens):
    ...
    separator = "------------------------\n"
    data['user_prompt'] = data.apply(lambda row: fit_prompt_to_window(user_prompt_instructions + separator + str(row['metadata_context']) + str(row['email_text_clean']) + separator, max_prompt_tokens), axis=1)
...
def classify_emails_node(model, data, max_tokens, temperature):
    ...
    data[source_column_name] = data.progress_apply(lambda row: classify_email(model, system_prompt, row['user_prompt'], max_tokens, temperature), axis=1)
    ...
    print(f"num patch entities to verify {data.shape[0]}")
    return data[['id', source_column_name, 'post_type','evaluated_keywords','evaluated_noun_chunks','unique_tokens','user_prompt','conversation_link','email_text_clean','email_text']]
...
def batch_update_new_features_patch(data):
    if data.empty:
        ...
        for index, row in data.iterrows():
            ...
            conversation_link = str(row['conversation_link']) if not pd.isna(row['conversation_link']) else 'N/A'
            update_data = {
        ...
        logger.info(f"patch feature engineering to Mongo load complete. {num_matched} records matched and {num_affected} records affected.")
        return True

...
from kedro_workbench.utils.periodic_report_CVE_WEEKLY_v1_prompts import (
    seven_day_periodic_report_CVE_WEEKLY_v1_prompt_strings as prompt_strings,
)
from kedro_workbench.utils.report_utils import (
    format_build_numbers,
    sort_products,
    fetch_and_merge_kb_data,
    fetch_package_pairs,
    record_report_total,
    plot_running_average,
    clean_appendix_title,
    split_product_name_string,
    filter_source_documents,
    prepare_plot_data,
    generate_and_save_plot,
    collect_metadata,
    build_mongo_pipeline,
    fetch_documents,
    add_category_to_documents,
    convert_to_utc_datetime,
    fetch_cve_documents_by_product_patterns,
    identify_exclusive_cve_documents,
    fetch_product_build_documents_by_product_patterns,
    annotate_package_pairs,
...
from kedro_workbench.utils.sendgrid_utils import (
    get_all_lists,
    get_recipients_from_sendgrid_list,
    load_encoded_file,
)
...
def clean_bad_build_data(report_end_date, day_interval, document_limit):
    ...
    logger.info(
        f"Report period - end date: {report_end_date}, day interval: {day_interval}, document limit: {document_limit}"
    )
    # Convert report_end_date to datetime object
    utc = pytz.UTC
    report_end_date_dt = datetime.strptime(
        report_end_date + " 23:59:59", "%d-%m-%Y %H:%M:%S"
    )
    report_end_date_dt = utc.localize(report_end_date_dt)
    report_start_date_dt = report_end_date_dt - timedelta(days=day_interval)
    report_start_date_dt = report_start_date_dt.replace(
        hour=0, minute=0, second=0, microsecond=0
    )
    print(
        f"report_start_date_dt: {report_start_date_dt}, report_end_date_dt: {report_end_date_dt}\n----------------------------"
    )
    ...
    product_builds = mongo_product_builds.collection.find({"cve_id": {"$in": post_ids}})
    ...
    mongo_product_builds.collection.delete_many({"cve_id": {"$in": post_ids}})
    # Update operation to set the specified keys to empty arrays
    match_query = {"metadata.post_id": {"$in": post_ids}}
    update_operation = {
        "$set": {
    ...
    print(f"completed removing bad data:\n{result}")


def fetch_section_1_periodic_report_CVE_WEEKLY_v1_data(
    products_to_keep_dict, report_end_date, day_interval, document_limit
):
    ...
    for key, products_list in products_to_keep_dict.items():
        products_to_keep_dict[key] = [
            split_product_name_string(product) for product in products_list
        ]

    # print("windows 11 only")
    # Fetch Windows 11 exclusive documents
    windows_11_exclusive_docs, windows_11_exclusive_ids = (
        identify_exclusive_cve_documents(
            docstore_collection,
            report_start_date_dt,
            report_end_date_dt,
    ...
    section_1_data = (
        windows_shared_docs + windows_11_exclusive_docs + edge_exclusive_docs
    )
    ...
    logger.info(
        f"Fetching data from MongoDB section 1: {len(section_1_data)} documents."
    )
    mongo.client.close()
...
def fetch_section_1_periodic_report_CVE_WEEKLY_v1_product_build_data(
    cve_data, report_end_date, day_interval, products_to_keep_dict
):
    if len(cve_data) == 0:
        return pd.DataFrame()
    ...
    filtered_product_build_data_list = (
        fetch_product_build_documents_by_product_patterns(
            product_builds_collection,
            None,
            report_end_date_dt,
            cve_ids,
            products_to_keep_dict["windows_10"]
    ...
    logger.info(
        f"Fetching Product Build data from MongoDB for section 1: {product_build_df.shape[0]} documents."
    )
    mongo.client.close()
    return product_build_df


def fetch_section_1_periodic_report_CVE_WEEKLY_v1_update_packages_data(
    product_build_data,
):
    ...
    query = {"product_build_id": {"$in": product_build_ids}}
    ...
    if update_package_data_list:
        ...
        update_package_df = pd.DataFrame(
            columns=[
                "build_number",
                "published",
                "product_build_id",
    ...
    logger.info(
        f"Fetching Update Package data from MongoDB for section 1: {update_package_df.shape[0]} documents."
    )
    mongo.client.close()
...
def transform_section_1_periodic_report_CVE_WEEKLY_v1_data(
    msrc_docs,
    ...
    result_df["build_number_str"] = result_df["build_numbers"].apply(
        format_build_numbers
    )
    result_df["core_products"] = result_df["products"].apply(sort_products)
    result_df["kb_article_pairs"] = result_df["post_id"].apply(
        lambda x: fetch_and_merge_kb_data(x, product_build_df)
    )
    result_df["package_pairs"] = result_df["post_id"].apply(
        lambda x: fetch_package_pairs(x, product_build_df, update_package_df)
    )
    result_df = result_df.apply(
        lambda row: annotate_package_pairs(row, monitoring_collection), axis=1
    )
    result_df["package_pairs"] = result_df["package_pairs"].apply(
        remove_empty_package_pairs
    )

    cat_type = CategoricalDtype(
        categories=[
            "windows_11_exclusive",
            "windows_10_windows_11_shared",
            "microsoft_edge",
        ],
        ordered=True,
    )
    ...
    mongo.client.close()
    logger.info(
        f"Built primary dataframe with {result_df.shape[0]} documents to process."
    )
    return result_df


def build_periodic_report_CVE_WEEKLY_v1_data_container(
    report_end_date, day_interval, report_params
):
...
def build_user_prompt_data_periodic_report_CVE_WEEKLY_v1(data, metadata_keys):
    ...
    logger.info("CVE_WEEKLY_v1 prompts built.")
...
def fit_prompt_data_periodic_report_CVE_WEEKLY_v1(data, max_prompt_tokens):
    ...
    data["summarization_context"] = data.apply(
        lambda row: (
            f"Post metadata:\n---\n{row['metadata_context']}\n---\n"
            f"Post text:\n---\n{row['text']}"
        ),
...
def summarize_section_1_periodic_report_CVE_WEEKLY_v1(
    model, data, max_tokens, temperature
):
    if data.empty:
    ...
    if not data.empty:
        ...
        updated_data = updated_data.sort_values(
            by=["published", "post_id"], ascending=[True, False]
        )
        logger.info(
            f"LLM Summarization complete. {updated_data.shape[0]} msrc posts summarized."
        )

        return updated_data
    else:
        raise ValueError(
            "summarize_section_1_periodic_report_CVE_WEEKLY_v1 requires a non empty DataFrame as input."
        )


def calculate_section_1_periodic_report_CVE_WEEKLY_v1(
    report_end_date, day_interval, source_documents, weekdays_order
):
    ...
    source_documents = filter_source_documents(
        source_documents, report_end_date, day_interval
    )
    ...
    output_file_path = (
        f"data/08_reporting/periodic_report_CVE_WEEKLY_v1/plots/{filename}"
    )
...
"""
        ...
        records_df["published_dates"] = records_df["published_dates"].apply(
            lambda x: [datetime.strptime(date, "%d-%m-%Y") for date in x]
        )
        records_df = records_df.explode("published_dates").sort_values(
            by="published_dates", ascending=False
        )
        # print(f"records_df: {records_df.head()}")
    return records_df
...
"""
        ...
        records_df = pd.DataFrame(
            fix_records,
            columns=["fix_id", "fix_description", "mongo_ids", "published_dates"],
        )
        records_df["published_dates"] = records_df["published_dates"].apply(
            lambda x: [datetime.strptime(date, "%d-%m-%Y") for date in x]
        )
...
"""
        ...
        records_df["published_dates"] = records_df["published_dates"].apply(
            lambda x: [datetime.strptime(date, "%d-%m-%Y") for date in x] if x else x
        )
...
"""
        ...
        for record in patch_records:
            ...
            conversation_links_list = (
                [link for link in conversation_links if link is not None]
                if conversation_links
                else None
            )
            receivedDateTimes_list = (
                [
                    datetime.strptime(dateTime, "%Y-%m-%dT%H:%M:%S%z").strftime(
                        "%d-%m-%Y"
                    )
                    for dateTime in receivedDateTimes
                ]
                if receivedDateTimes
                else None
            )
            # Create a list of dictionaries for 'pm_fixes'
            fixes_list = (
                [
                    {"description": fix["description"], "url": fix.get("url")}
                    for fix in pm_fixes
                    if fix is not None
                ]
                if pm_fixes
            ...
            tools_list = (
                [
                    {"description": tool["description"], "url": tool.get("url")}
...
def calculate_section_6_periodic_report_CVE_WEEKLY_v1(community_data):
    ...
    total_tools = (
        community_data["pm_tools"].apply(lambda x: len(x) if x is not None else 0).sum()
    )
    total_fixes = (
        community_data["pm_fixes"].apply(lambda x: len(x) if x is not None else 0).sum()
    )
    community_metadata = {
        "total_contributions": total_tools + total_fixes,
        "community_fixes": total_fixes,
        "community_tools": total_tools,
    }
...
def compile_periodic_report_CVE_WEEKLY_v1(
    section_1_data,
    section_2_data,
    symptom_data,
    fix_data,
    ...
    for column in columns_to_convert:
        if column in section_1_df.columns:
            section_1_df[column] = section_1_df[column].apply(
                lambda x: ast.literal_eval(x) if pd.notnull(x) else x
            )
        else:
            logger.info(
                f"compile_periodic_report: Column '{column}' not found in the DataFrame."
            )
    ...
    section_1_df = section_1_df.sort_values(
        ["report_category", "post_id"], ascending=[True, False]
    )
    section_1_df["summarization_payload"] = section_1_df["summarization_payload"].apply(
        lambda x: "" if isinstance(x, float) else x
    )
    # print('after section_1_data tinkering\n')
    # for id, row in section_1_df.iterrows():
    #     id = row['post_id']
    #     category = row['report_category']
    #     package_pairs = row['package_pairs']
    #     print(f"{id} - {package_pairs}\n\n")
    section_1_df["revision"] = section_1_df["revision"].apply(
        lambda x: "{:.1f}".format(x) if pd.notna(x) else "1.0"
    )
    section_1_df["published"] = section_1_df["published"].dt.strftime("%d-%m-%Y")
    section_1_df["published"] = section_1_df["published"].astype(str)

    section_1_df.drop(
        columns=["metadata_context", "user_prompt", "weekday"], inplace=True
    )
    ...
    if not section_2_df.empty:
        ...
        merged_section_1_df = pd.merge(
            section_1_df,
            exploded_section_2_df,
            left_on="id",
            right_on="mongo_id",
            how="left",
        )
        grouped_df = merged_section_1_df.groupby("id")["product_name"].agg(
            lambda x: ", ".join(x.dropna().unique())
        )
        section_1_df["products"] = section_1_df["id"].map(grouped_df)
        unique_section_2_df = (
            section_2_df.groupby(["product_name", "product_version"])["product_id"]
            .agg(list)
            .reset_index()
        )
    ...
    if not symptoms_df.empty:
        ...
        grouped_df = merged_section_1_df.groupby("id")["symptom_description"].agg(
            lambda x: ", ".join(x.dropna().unique())
        )
    ...
    if not fixes_df.empty:
        ...
        grouped_df = merged_section_1_df.groupby("id")["fix_description"].agg(
            lambda x: ", ".join(x.dropna().unique())
        )
    ...
    if not section_3_df.empty:
        section_3_df.dropna(
            subset=["tool_description", "tool_url"], how="all", inplace=True
        )
        exploded_section_3_df = section_3_df.explode("mongo_ids")
        merged_section_1_df = pd.merge(
            section_1_df,
            exploded_section_3_df,
            left_on="id",
            right_on="mongo_ids",
            how="left",
        )
        grouped_df = merged_section_1_df.groupby("id")["tool_description"].agg(
            lambda x: ", ".join(x.dropna().unique())
        )
    ...
    if not community_df.empty:
        section_1_df = pd.merge(
            section_1_df, community_df, left_on="id", right_on="mongo_id", how="left"
        )
        ...
        for col in list_columns:
            section_1_df[col] = section_1_df[col].apply(
    ...
    report_end_date_dt = datetime.strptime(
        report_data_container["report_end_date"] + " 23:59:59", "%Y_%m_%d %H:%M:%S"
    )
    report_end_date_dt = utc.localize(report_end_date_dt)
    ...
    report_start_date_dt = datetime.strptime(start_date_str, "%Y_%B_%d").replace(
    ...
    pipeline = [
        {
            "$match": {
                "metadata.collection": {"$in": appendix_collections},
                "metadata.published": {
                    "$gte": report_start_date_dt,
                    "$lte": report_end_date_dt,
                },
        ...
        {"$group": {"_id": "$metadata.collection", "documents": {"$push": "$$ROOT"}}},
        {"$sort": {"_id": 1}},  # Optional: Sort the groups by collection name if needed
    ]
    ...
    for group in results:
        ...
        for doc in group["documents"]:
            ...
            title_or_subject = metadata.get(
                "title", metadata.get("subject", "No Title or Subject")
            )

            cleaned_title_or_subject = clean_appendix_title(title_or_subject)
            source_with_base_url = add_base_url_to_source(
                metadata.get("source", "No Source"), group["_id"]
            )
            reshaped_document = {
...
def save_periodic_report_CVE_WEEKLY_v1_data(report_data_container):
    ...
    csv_file_name = (
        f"data/08_reporting/periodic_report_CVE_WEEKLY_v1/metadata/weekly_totals.csv"
    )
    ...
    if len(report_data_container["section_1_data"]) > 0:
        ...
        record_report_total(
            timestamp,
            report_data_container["section_1_metadata"]["total_cves"],
            csv_file_name,
        )
        ...
        weekly_summary_plot_file_path = (
            f"data/08_reporting/periodic_report_CVE_WEEKLY_v1/plots/"
        )
        weekly_summary_plot_file_name = f"weekly_totals_{timestamp}.png"
        full_path = os.path.join(
            weekly_summary_plot_file_path, weekly_summary_plot_file_name
        )
...
def generate_periodic_report_CVE_WEEKLY_v1_html(report_data_container):
    ...
    if len(report_data_container["section_1_data"]) > 0:
        ...
        # section_1 = report_data_container["section_1_data"]
        # print(f"in generate html:\n")

        # for d in section_1:
        #     print(f"{d['post_id']} - {d['package_pairs']}")
        try:
            # Template directory
            ...
            logger.info(f"HTML report generated successfully. {full_html_path}")
            # create thumbnail for report
            thumbnail_path = (
                "data/08_reporting/periodic_report_CVE_WEEKLY_v1/thumbnails/"
            )
...
def send_notification_to_sendgrid_qa_list(report_data_container, params):
    ...
    report_qa_sendgrid_list = next(
        (d for d in all_lists if d["name"] == "msrc_weekly_report_qa_team"), None
    )
    ...
    notification_to = get_recipients_from_sendgrid_list(
        sendgrid_api_key, report_qa_sendgrid_list_id
    )
    ...
    attachment = Attachment(
        file_content=FileContent(report_thumbnail),
        file_type=FileType("image/png"),
        file_name=FileName(extract_file_name(thumbnail_path)),
...
def create_draft_campaign_cve_weekly(report_data_container, params):
    ...
    # Fetch configuration and list details
    all_lists = get_all_lists(sendgrid_api_key)
    report_subscriber_sendgrid_list = next(
        (d for d in all_lists if d["name"] == "msrc_weekly_report"), None
    )
    ...
    patch_tuesday = (
        "[Patch Tuesday]"
        if is_second_tuesday(datetime.strptime(report_end_date, "%Y_%m_%d"))
        else ""
    )
    # print(f"patch_tuesday: {patch_tuesday}")
    campaign_subject = campaign_subject_template.format(
        report_end_date=report_end_date, patch_tuesday=patch_tuesday
    )
...
def move_cve_weekly_report_assets_to_blob(report_data_container):
    ...
    if report_data_container:
        ...
        logger.info(
            f"No report generated. Skipping moving report assets to blob storage."
        )
...
def load_report_assets_to_webserver(report_data_container, params):
    ...
    if report_data_container:
        ...
        file_mappings = build_file_mappings(sftp_payload, remote_base_path)

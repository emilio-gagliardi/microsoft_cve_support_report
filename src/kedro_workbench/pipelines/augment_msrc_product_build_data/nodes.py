...
def merge_product_build_data_with_msrc_docs(
    new_product_build_data,
    existing_docs_to_augment,
    columns_to_keep
):
...
def load_augmented_msrc_posts_product_build_docs(updated_msrc_docs):
    ...
    for cve in updated_msrc_docs:
        ...
        for existing_doc in existing_docs:
            ...
            # The MongoDB $addToSet operator is used to ensure that only unique values are added to the array. $each
            # setdefault Retrieves the value for the specified key if it exists. If the key does not exist, it inserts the key with the specified default value and then returns that default value. dict.get('key', 'default value') does not modify the dict

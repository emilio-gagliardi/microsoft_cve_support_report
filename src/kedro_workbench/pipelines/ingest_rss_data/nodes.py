...
def transform_rss_1_feed(
    ...
    sorted_transformed_rss_feed = sorted(
        transformed_rss_feed, key=lambda x: x["post_id"], reverse=True
    )
    ...
    if not skip_download:
        ...
        product_patterns_windows_10 = all_params["product_build_product_patterns"][
            "windows_10"
        ]
        product_patterns_windows_11 = all_params["product_build_product_patterns"][
            "windows_11"
        ]
        ...
        node_status = download_windows10_product_build_data(
            skip_download, download_params, headless=True, begin_ingestion=True
        )
        ...
        csv_data_edge = LatestExcelDataSet(
            filepath="data/01_raw/edge/", load_args={"sheet_name": "Security Updates"}
        )
        ...
        windows_10_df = extract_filter_product_build_data(
            data_win10, product_patterns_windows_10
        )
        ...
        windows_11_df = extract_filter_product_build_data(
            data_win11, product_patterns_windows_11
        )
        # We are only interested in cataloguing the unique CVEs here
        # We only need unique cve data, so we drop duplicates
        ...
        concatenated_df = pd.concat(
            [windows_10_df, windows_11_df, edge_df], ignore_index=True
        )
        ...
        concatenated_df["Release date"] = pd.to_datetime(
            concatenated_df["Release date"], format="%b %d, %Y"
        )
        concatenated_df["Release date"] = concatenated_df["Release date"].dt.strftime(
            "%a, %d %b %Y 00:00:00 Z"
        )
        ...
        category = "CVE"
        # print(f"transform_rss_1 count: {concatenated_df.shape[0]}")
        for idx, row in concatenated_df.iterrows():
            temp_dict = {}
            source_url = row.get(
                "source",
                f"https://msrc.microsoft.com/update-guide/vulnerability/{row.get('post_id', '')}",
            )

            published = row["published"]
            post_id = row["post_id"]
            # print(f"processing {post_id} - {source_url}\n")
            if not source_url:
                logger.info(
            ...
            header_element = wait_for_selenium_element(
                # Microsoft chanegd their stylesheet in July 2024
                driver,
                "h1.ms-fontWeight-semibold.css-174",
            )
            if not header_element:
                # Microsoft chanegd their stylesheet in July 2024
                header_element = wait_for_selenium_element(
                    driver, "h1.ms-fontWeight-semibold.css-196"
                )
            if not header_element:
                print("trying general h1...")
                header_element = wait_for_selenium_element(driver, "h1")
            ...
            header_text = (
                ...
                if header_element
                else "MSRC Title not found"
            )

            revisions_grid = wait_for_selenium_element(
                driver, "div[role='grid'][aria-label='Revisions']"
            )
            if revisions_grid:
                ...
                if first_row:
                    ...
                    if version_element:
                        version_text = execute_js_on_element(
                            driver, version_element, "return arguments[0].innerText;"
                        )
                    else:
                        version_text = "0.9"
            ...
            sorted_downloaded_cves = sorted(
                downloaded_cves, key=lambda x: x["post_id"], reverse=True
            )
            # print("EXCEL CVEs")

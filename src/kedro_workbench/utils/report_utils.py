...
def build_mongo_pipeline(start_date, end_date, products_list):
    logger.info(
        f"Building pipeline to fetch MSRC documents for start_date: {start_date}, end_date: {end_date}, products_list: {products_list}"
    )
    return [
        {
            "$match": {
...
def fetch_cve_documents_by_product_patterns(
    collection, start_date, end_date, patterns, exclude_ids=[], use_regex=False
):
    if use_regex:
        # print(f"fetch_cves with regex: {patterns}")
        # Compile each pattern into a regex object
        regex_objects = [
            re.compile(f"\\s*{re.escape(pattern)}\\s*", re.IGNORECASE)
            for pattern in patterns
        ]
        product_query = {
            "$regex": "|".join(
                [f"\\s*{re.escape(pattern)}\\s*" for pattern in patterns]
            ),
            "$options": "i",
        }
        ...
        ]  # Compile for non-regex string match
...
def identify_exclusive_cve_documents(
    collection, start_date, end_date, include_patterns, exclude_patterns
):
    """Identify documents that match include patterns but not exclude patterns."""
    include_docs = fetch_cve_documents_by_product_patterns(
        collection, start_date, end_date, include_patterns
    )
    # print("---include---")
    # for doc in include_docs:
    #     print(f"{doc['metadata']['id']}-{doc['metadata']['source']}")
    exclude_docs = fetch_cve_documents_by_product_patterns(
        collection, start_date, end_date, exclude_patterns
    )
    # print("---exclude---")
    ...
    exclusive_docs = [
        doc for doc in include_docs if doc["metadata"]["id"] not in exclude_ids
    ]
...
def fetch_product_build_documents_by_product_patterns(
    collection, start_date, end_date, cve_ids, patterns, exclude_ids=[], use_regex=False
):
    """
    ...
    query = {"cve_id": {"$in": cve_ids}, "product_build_id": {"$nin": exclude_ids}}
...
def fetch_and_merge_kb_data(post_id, product_build_df):
    ...
    for _, row in dataframe_slice.iterrows():
        ...
        published_date = row[
            "published"
        ]  # Assuming 'published' is already a datetime object
        published_str = published_date.strftime("%d-%m-%Y")
        article_url = row.get(
            "article_url"
        )  # Adjust 'article_url' if the column name is different
    ...
    unique_pairs = {
        (pair["kb_id"], pair["kb_link"]): pair for pair in kb_article_pairs
    }.values()
...
def fetch_package_pairs(post_id, product_build_df, update_package_df):
    ...
    update_package_data_for_cve = update_package_df[
        update_package_df["product_build_id"].isin(product_build_ids)
    ]
    ...
    for _, package_row in update_package_data_for_cve.iterrows():
        ...
        if "downloadable_packages" in package_row and isinstance(
            package_row["downloadable_packages"], list
        ):
            for downloadable_package in package_row["downloadable_packages"]:
                ...
                downloadable_package_dict = {
                    ...
                    "product_architecture": downloadable_package.get(
                        "product_architecture", ""
                    ),
                    "update_type": downloadable_package.get("update_type", ""),
                    "file_size": downloadable_package.get("file_size", ""),
                    "restart_behavior": downloadable_package.get(
                        "restart_behavior", ""
                    ),
                    "request_user_input": downloadable_package.get(
                        "request_user_input", ""
                    ),
                    "installed_exclusively": downloadable_package.get(
                        "installed_exclusively", ""
                    ),
                    "requires_network": downloadable_package.get(
                        "requires_network", ""
                    ),
                    "uninstall_notes": downloadable_package.get("uninstall_notes", ""),
                }
                existing_packages = packages_by_url[package_url][
                    "downloadable_packages"
                ]
                if not any(
                    downloadable_package_dict.items() <= existing.items()
                    for existing in existing_packages
                ):
                    packages_by_url[package_url]["downloadable_packages"].append(
                        downloadable_package_dict
                    )
...
def fetch_baseline_value(monitoring_collection, field_name):
    ...
    monitoring_document = monitoring_collection.find_one(
        {"field_name": field_name}, {"values": 1}
    )
...
def annotate_package_pairs(
    row, monitoring_collection, field_to_check="restart_behavior"
):
    ...
    if "package_pairs" in row and isinstance(row["package_pairs"], list):
        for package_pair in row["package_pairs"]:
            ...
            if "downloadable_packages" in package_pair and isinstance(
                package_pair["downloadable_packages"], list
            ):
...
def filter_dataframe(
    df, start_date, end_date, columns_to_keep=["item_total", "running_average"]
):
...
def plot_data(ax, df, line_colors, trendline_color):
    ...
    columns_to_keep = [
        "item_total",
        "running_average",
    ]  # These should be the actual column names in your dataframe
    line_labels = [
        "Number of Posts",
        "Average",
    ]  # Labels used for annotation, should correspond to columns_to_keep
...
def annotate_lines(ax, df, line_labels, line_colors, trendline_color):
    columns_to_keep = [
        "item_total",
        "running_average",
    ]  # The actual column names from the dataframe
    ...
    for i, col in enumerate(columns_to_keep):
        ...
        ax.annotate(
            line_labels[i],  # Use the descriptive label for annotation
            xy=(df.index[-1], y_val),
...
def adjust_tick_params(ax):
    ...
    ax.tick_params(
        axis="x", which="major", length=10, labelsize=10, labelcolor="black", pad=31
    )
    ax.tick_params(
        axis="x", which="minor", length=5, labelsize=8, labelcolor="gray", pad=5
    )
...
def plot_running_average(
    ...
    annotate_lines(
        ax, filtered_df, ["Number of Posts", "Average"], line_colors, trendline_color
    )
...
def filter_source_documents(source_documents, report_end_date, day_interval):
    ...
    source_documents["published"] = pd.to_datetime(
        source_documents["published"], dayfirst=True
    )
    filtered_docs = source_documents[
        (source_documents["published"] >= start_date)
        & (source_documents["published"] <= end_date)
    ]
...
def adjust_weekday_for_weekends(source_documents):
    ...
    source_documents.loc[source_documents["weekday"] == "Saturday", "weekday"] = (
        "Monday"
    )
...
def prepare_plot_data(source_documents, new_order):
    ...
    final_df["weekday"] = pd.Categorical(
        final_df["weekday"], categories=new_order, ordered=True
    )
...
def generate_and_save_plot(final_df, output_file_path, report_end_date, day_interval):
    ...
    date_range = (
        f"({start_date.strftime('%B %d')} - {report_end_date_dt.strftime('%B %d')})"
    )
    ...
    if "Monday" in final_df["weekday"].values:
        ...
        ax.annotate(
            f"Total: {int(total_posts)}",
            xy=(max_count, monday_y),
            xycoords="data",
            ha="right",
            va="center",
            fontsize=15,
...
def create_thumbnail(html_file_path, thumbnail_path, width_cm=6, headless=True):
    ...
    # print("Initializing create_thumbnail function")
    ...
    try:
        ...
        # print(f"Opening HTML file at {html_file_path}")
    ...
    try:
        ...
        thumbnail_image = screenshot_image.resize(
            (width_px, height_px), Image.Resampling.LANCZOS
        )
        thumbnail_image.save(thumbnail_path)
        # print(f"Thumbnail saved successfully at {thumbnail_path}")
    except Exception as e:

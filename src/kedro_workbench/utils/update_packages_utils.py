...
def wait_for_results(driver, class_name, timeout=20):
    try:
        ...
        print(f"Selenium timed out after {timeout} sec waiting for {class_name} to load.")
    except Exception as e:
...
def process_matching_link(link, driver, original_window, downloadable_package_details):
    ...
    try:
        ...
        WebDriverWait(driver, 10).until(EC.visibility_of_element_located((By.ID, "installDetails")))
    except TimeoutException:
    ...
    downloadable_dict = {"product_name":f"windows_{product_version}", "product_version":version, "product_architecture":architecture, "update_type":update_type, "install_resources_text":install_info_text, "install_resources_html":html_content, "last_modified":last_modified, "file_size": file_size}
    if downloadable_package_details.get('product_version') == None or downloadable_package_details.get('product_version') == 'None':
...
def matches_criterion(downloadable_package_details, criterion):
    ...
    if criterion.get('version') is None:
        ...
        version_matches = criterion['version'] == downloadable_package_details.get('version', '')
...
def process_downloadable_package(link, driver, original_window, criterion):
    ...
    if not matches_criterion(downloadable_package_details, criterion):
        ...
        return None
    # print(f"=== criterion matched downloadable_package_details ===\n")
    # Processing the link if it matches the criterion
    downloadable_dict = process_matching_link(link, driver, original_window, downloadable_package_details)
...
def find_downloadable_packages_for_update_package(driver, original_window, search_criteria, css_selector):
...
def display_execution_time(start_time):
    ...
    logger.info(f"Augmenting update package data Execution time: {int(minutes)} minutes and {seconds:.2f} seconds.")
...
def fetch_existing_update_packages(update_package_docs):
    ...
    urls = set(doc['package_url'] for doc in update_package_docs if doc.get('package_url'))
...
def process_downloadable_package_additional_details(doc, search_criteria, headless=True):
    ...
    if not wait_for_results(driver, "resultsbottomBorder", 10):
        print("Selenium driver could not find td that holds downloadable packages. Return to node.")
        driver.close()
        time.sleep(3)
        driver.quit()
        return []
        ...
        for package in doc['downloadable_packages']:
            ...
            if 'restart_behavior' not in package:
                ...
                if html_content:
                    ...
                    package['restart_behavior'] = soup.find(id="ScopedViewHandler_rebootBehavior").get_text(strip=True) if soup.find(id="ScopedViewHandler_rebootBehavior") else "Unknown"
                    package['request_user_input'] = soup.find(id="ScopedViewHandler_userInput").get_text(strip=True) if soup.find(id="ScopedViewHandler_userInput") else "Unknown"
                    package['installed_exclusively'] = soup.find(id="ScopedViewHandler_installationImpact").get_text(strip=True) if soup.find(id="ScopedViewHandler_installationImpact") else "Unknown"
                    package['requires_network'] = soup.find(id="ScopedViewHandler_connectivity").get_text(strip=True) if soup.find(id="ScopedViewHandler_connectivity") else "Unknown"
                    
                    # Extract 'uninstall_notes' and remove the "Uninstall Notes:" prefix
                    uninstall_notes_text = soup.find(id="uninstallNotesDiv").get_text(strip=True) if soup.find(id="uninstallNotesDiv") else "Unknown"
                    # Replace only the first occurrence
                    package['uninstall_notes'] = uninstall_notes_text.replace("Uninstall Notes:", "", 1)  

                    uninstall_steps_text = soup.find(id="uninstallStepsDiv").get_text(strip=True) if soup.find(id="uninstallStepsDiv") else "Unknown"
                    # Replace only the first occurrence
                    package['uninstall_steps'] = uninstall_steps_text.replace("Uninstall Steps:", "", 1)           
    else:
        logger.info(f"WARNING. No 'downloadable_packages' found in Update Package with id {doc['id']}\n")
